...
trait Preliminaries extends TwelfSignature {
  ...
  val add   = 'add   :> nat ->: nat ->: nat ->: Type
  val add_z = 'add_z :> add(z)(N)(N)
  val add_s = 'add_s :> add(N1)(N2)(N3) ->: add(s(N1))(N2)(s(N3))

  val lte   = 'lte   :> nat ->: nat ->: Type
  val lte_z = 'lte_z :> lte (z) (N)
  val lte_s = 'lte_s :> lte (N1) (N2) ->: lte (s(N1)) (s(N2))

  val id_nat     = 'id_nat     :> nat ->: nat ->: Type
  val id_nat_rfl = 'id_nat_rfl :> id_nat (X) (X)

  val eq_nat   = 'eq_nat   :> nat ->: nat ->: Type
  val eq_nat_z = 'eq_nat_z :> eq_nat (z) (z)
  val eq_nat_s = 'eq_nat_s :> eq_nat (N1) (N2) ->: eq_nat (s (N1)) (s (N2))

  val s_injective = 's_injective :> id_nat (N1) (N2) ->: id_nat (s (N1)) (s (N2)) ->: Type
  % { mode (s_injective) ++(A) --(B) }

  val s_injective_rfl = 's_injective_rfl :> s_injective (id_nat_rfl) (id_nat_rfl)
  % { worlds (s_injective) ('_) ('_) }
  % { total (A) (s_injective) (A) ('_) }

  val eq2id_nat = 'eq2id_nat :> eq_nat (N1) (N2) ->: id_nat (N1) (N2) ->: Type
  % { mode (eq2id_nat) ++(A) --(B) }
  val eq2id_nat_z = 'eq2id_nat_z :> eq2id_nat (eq_nat_z) (id_nat_rfl)
  val eq2id_nat_s = 'eq2id_nat_s :> s_injective (B) ('OUT) ->:
                                    eq2id_nat (A) (B) ->:
                                    eq2id_nat (eq_nat_s (A)) ('OUT)
  % { worlds (eq2id_nat) ('_) ('_) }
  % { total (A) (eq2id_nat) (A) ('_) }

  val eq_nat_rfl = 'eq_nat_rfl :> !!(N, nat)/ { eq_nat (N) (N) } ->: Type
  % { mode (eq_nat_rfl) ++(N) --('EQ) }
  val eq_nat_rfl_z = 'eq_nat_rfl_z :> eq_nat_rfl (z) (eq_nat_z)
  val eq_nat_rfl_s = 'eq_nat_rfl_s :> eq_nat_rfl (N) ('EQ) ->:
                                      eq_nat_rfl (s (N)) (eq_nat_s ('EQ))
  % { worlds (eq_nat_rfl) ('_) ('_) }
  % { total (N) (eq_nat_rfl) (N) ('_) }

  val id2eq_nat = 'id2eq_nat :> id_nat (N1) (N2) ->: eq_nat (N1) (N2) ->: Type
  % { mode (id2eq_nat) ++(A) --(B) }

  '- :> eq_nat_rfl ('_) ('EQ) ->: id2eq_nat (id_nat_rfl) ('EQ)
  % { worlds (id2eq_nat) ('_) ('_) }
  % { total (A) (id2eq_nat) (A) ('_) }

  val add_inc = 'add_inc :> add (A) (B) (C) ->: add (A) (s (B)) (s (C)) ->: Type
  % { mode (add_inc) ++('E1) --('E2) }
...
trait Generics extends LFList with Preliminaries {
  override def listOf(t: Family) = {
    ...
    list_size_cons :> list_size ('L) ('N) ->: list_size (cons ('_) ('L)) (s('N))
    ...
    sub_list_ext :> sub_list (L1) (L2) ->: sub_list (L1) (cons ('_) (L2))
    ...
    % { worlds (list_size_eq) ('_) ('_) ('_) }
    % { total (A) (list_size_eq) (A) ('_) ('_) }
    ...
    % { worlds (list_size_id) ('_) ('_) ('_) }
    % { total (A) (list_size_id) (A) ('_) ('_) }
...
trait Repetitions extends RecursiveExtensions {
  println(TwelfPrinter(this.bindings))
}
